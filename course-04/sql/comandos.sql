...
-- Criando exemplos de funções utilizando o banco academico
SELECT * FROM academico.aluno;
SELECT * FROM academico.categoria;
SELECT * FROM academico.curso;
SELECT * FROM academico.aluno_curso;

CREATE FUNCTION academico.cria_curso(nome_curso VARCHAR, nome_categoria VARCHAR) RETURNS void AS $$
	DECLARE id_categoria INTEGER;
	BEGIN
		-- BUSCAR ID DA CATEGORIA ATRAVÉS DO NOME DA CATEGORIA
		SELECT id INTO id_categoria
			FROM academico.categoria WHERE academico.categoria.nome = nome_categoria;
		-- VERIFICAR SE A CATEGORIA JÁ EXISTE
		IF NOT FOUND THEN
			-- SE NÃO HOUVER A CATEGORIA, INSERIR A CATEGORIA E RESGATAR O ID
...
SELECT * FROM academico.cria_curso('Introdução à Javascript', 'Tecnologia');
SELECT * FROM academico.cria_curso('Conhecendo Gênesis', 'Religião');

-- Verificar a estrutura da tabela instrutor
SELECT * FROM instrutor;
...
DECLARE 
    ...
    percentual DECIMAL;
BEGIN
    -- Busca a média de salário dos instrutores
    SELECT AVG(instrutor.salario) INTO media_salarial FROM instrutor;
    
    -- Insere novo instrutor e recupera o seu ID
    INSERT INTO instrutor (nome, salario) 
        VALUES (nome_instrutor, salario_instrutor) RETURNING id INTO id_instrutor_inserido;

    -- Verifica se o salário do instrutor é maior que a média salarial e, se for, insere o log
    IF salario_instrutor > media_salarial THEN
        INSERT INTO log_instrutores (informacao) 
            VALUES (nome_instrutor || ' recebe acima da média');
    END IF;

    -- Verifica quantos instrutores recebem menos que o novo instrutor
    ...
    -- Calcula o percentual de instrutores que recebem menos que o novo instrutor e insere no log
...
-- Testando a função cria_instrutor
SELECT cria_instrutor('Josiel Alemão', 1000);

-- Verificar as tabelas após a execução da função
SELECT * FROM instrutor;
